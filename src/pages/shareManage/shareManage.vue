...
export default {
  ...
  methods: {
    del(id) {
      ...
      vm
        .$confirm("确定要删除该分享?", "提示", { type: "warning" })
        .then(() => {
          vm.isloading = !0;
          vm.$http
            .get(
              `${vm.serverUrl}share/delete-${
                vm.current == 0 ? "utest" : "project"
              }-share/`,
              {
                params: {
                  share_id: id,
    ...
    getData(page) {
      ...
      vm.$http
        .get(`${serverUrl}share/my-utest-share/`, {
          params: {
            token: vm.token,
            page_idx: vm.page,
            page_size: 30
        ...
        .then(res => {
          ...
          vm.isloading = !1;
          vm.data = data.page_data;
          vm.totalPage = data.page_total_cnts;
        })
        .catch(onfail);
    },
    getData2(page) {
      ...
      vm.$http
        .get(`${serverUrl}share/my-project-share/`, {
          params: {
            token: vm.token,
            page_idx: vm.page2,
            page_size: 30
